...
def get_reviews(hyphen_restaurants, city):
    non_valid = []
    reviews = {}
    for restaurant in hyphen_restaurants[-11:]:
        print(restaurant)
        # create the restaurant's url using the hyphenated names
        ...
        # skips restaurant if not in city
        if first_ten == []:
            print(f"{restaurant} is not in {city}")
            non_valid.append(restaurant)
            continue 

        # get the number of reviews using regex
...
def main():
    ...
    get_reviews(hyphenated_restaurants, 'detroit')
    print('done')
